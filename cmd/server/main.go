...
func main() {
	...
	// Initialize LLM service for smart analysis
	groqService := llm.NewGroqService(cfg.GroqAPIKey)
	unifiedProcessor.GetUnifiedService().SetLLMService(groqService)
...
func registerAdapters(cfg *config.Config) {
	...
	// FPT AI ASR key from user request
	fptAPIKey := "tN4aZtZo0u2cNZj9Obzv8SO99y0vFiIr"
	registry.RegisterTranscriptionAdapter("fpt_ai",
		adapters.NewFPTAdapter(fptAPIKey))

	// Register Groq adapter
	registry.RegisterTranscriptionAdapter("groq_whisper",
		adapters.NewGroqAdapter(cfg.GroqAPIKey))
