...
func (h *Handler) SubmitQuickTranscription(c *gin.Context) {
	...
	saveToPortalStr := c.PostForm("save_to_portal")
	saveToPortal := saveToPortalStr == "true"

	fmt.Printf("!!! [BACKEND] Processing session: %s (SaveToPortal: %v)\n", sessionID, saveToPortal)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Audio file is required"})
		return
	}
	...
	// Parse optional parameters from form
	if model := c.PostForm("model"); model != "" {
		params.Model = model
	}
	if family := c.PostForm("model_family"); family != "" {
		params.ModelFamily = family
	}
	// else if params.ModelFamily == "" {
	// 	params.ModelFamily = "fpt" // Default to FPT for quick jobs -> REMOVED to allow auto-selection
	// }
