...
// InitializeModels ensures registered models are ready to use
func (r *ModelRegistry) InitializeModels(ctx context.Context) error {
	...
	lowMemoryMode := strings.ToLower(os.Getenv("LOW_MEMORY_MODE")) == "true"

	var enabledModels map[string]bool
	if enabledModelsStr != "" {
		enabledModels = make(map[string]bool)
		for _, m := range strings.Split(enabledModelsStr, ",") {
			enabledModels[strings.TrimSpace(m)] = true
		}
	}

	logger.Info("Initializing registered models...")

	// Helper function to initialize an adapter
	initAdapter := func(id string, adapter interface {
		GetCapabilities() interfaces.ModelCapabilities
		PrepareEnvironment(context.Context) error
	}, typeName string) error {
		caps := adapter.GetCapabilities()

		// Skip if selective initialization is enabled and this model isn't in the list
		if enabledModels != nil && !enabledModels[id] {
			logger.Debug(fmt.Sprintf("Skipping %s model (not in ENABLED_MODELS)", typeName), "model_id", id)
			return nil
		}

		// Skip heavy models in low memory mode
		if lowMemoryMode && caps.MemoryRequirement > 0 {
			logger.Info(fmt.Sprintf("Skipping heavy %s model (LOW_MEMORY_MODE active)", typeName), "model_id", id)
			return nil
		}
		...
		if err := adapter.PrepareEnvironment(ctx); err != nil {
			...
			return fmt.Errorf("%s model %s: %w", typeName, id, err)
		}
	...
	for modelID, adapter := range r.transcriptionAdapters {
		if err := initAdapter(modelID, adapter, "transcription"); err != nil {
	...
	for modelID, adapter := range r.diarizationAdapters {
		// Use a temporary interface wrapper since diarization adapters also have GetCapabilities
		type capAdapter interface {
			GetCapabilities() interfaces.ModelCapabilities
			PrepareEnvironment(context.Context) error
		}
		if err := initAdapter(modelID, adapter.(capAdapter), "diarization"); err != nil {
			errorList = append(errorList, err)
		}
	}
	...
	for modelID, adapter := range r.compositeAdapters {
		if err := initAdapter(modelID, adapter, "composite"); err != nil {
			errorList = append(errorList, err)
		}
	}
