...
func (w *WhisperXAdapter) PrepareEnvironment(ctx context.Context) error {
	...
	// Force cleanup of potentially corrupted venv if check failed
	logger.Info("WhisperX environment check failed, forcing clean install", "path", whisperxPath)
	os.RemoveAll(filepath.Join(whisperxPath, ".venv"))
	// Also remove cache to be safe
	os.RemoveAll(filepath.Join(whisperxPath, ".uv_cache"))
