...
// Defaults to "cu126" for CUDA 12.6 if a GPU is detected, otherwise "cpu".
...
func GetPyTorchCUDAVersion() string {
	...
	// Try to detect if we have a GPU
	if _, err := os.Stat("/dev/nvidia0"); err == nil {
		return "cu126" // Default to CUDA 12.6 if GPU exists
	}
	...
	return "cpu" // No GPU detected, default to CPU
}

// GetPyTorchWheelURL returns the full PyTorch wheel URL for the configured CUDA version.
func GetPyTorchWheelURL() string {
	version := GetPyTorchCUDAVersion()
	if version == "cpu" {
		return "https://download.pytorch.org/whl/cpu"
	}
	return fmt.Sprintf("https://download.pytorch.org/whl/%s", version)
}
