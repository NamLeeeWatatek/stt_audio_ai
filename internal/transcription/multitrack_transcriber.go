...
// IsJobMultiTrack checks if a job is currently being tracked as a multi-track job
func (mt *MultiTrackTranscriber) IsJobMultiTrack(jobID string) bool {
	mt.trackJobsMutex.RLock()
	defer mt.trackJobsMutex.RUnlock()
	_, exists := mt.activeTrackJobs[jobID]
	return exists
}
